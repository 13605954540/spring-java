...
public class CodeBuildUtil {
    ...
    private String path = "scq";

    private String authorName = "Lin";

    private String tableName = "db_month";
    ...
    //生成get set方法
    private StringBuffer getMethod(StringBuffer sb){
        for(int i = 0;i<colNames.length;i++){
            sb.append("\tpublic void set" + initClass(colNames[i]) + "(" + getJaveType(colTypes[i]) + " " + getPropertyName(colNames[i]) + "){\r\n");
            sb.append("\t\tthis." + getPropertyName(colNames[i]) + " = " + getPropertyName(colNames[i]) + ";\t\n");
            sb.append("\t}\r\n\n");
            sb.append("\tpublic " + getJaveType(colTypes[i]) + " get" + initClass(colNames[i]) + "(){\r\n");
            sb.append("\t\treturn " + getPropertyName(colNames[i]) + ";");
            sb.append(i == colNames.length - 1 ? "\r\t}\n" : "\r\t}\r\n\n");
        }
        return sb;
    }
    ...
    //执行输出
    public CodeBuildUtil() throws ClassNotFoundException, SQLException, IOException {
        ...
        File dir = new File("");
        String outPath = dir.getAbsolutePath() + "/src/" + this.path.replace(".","/") + "/" + initClass(tableName) + ".java";
